...
//SPI Devices Initialization=====================================
  ...
  mpu6050.begin();      //MPU
   ...
   //
//    To add a logic that will check if backup exists but those two files were empty
    //
  bool prevDataCalibrate = false;  
  if(backUpThere){
    prevDataCalibrate = callibrateUsingPrevDatafromSD();
    setPacketCountFromSD();
  }
  if(!backUpThere || !prevDataCalibrate){  //if There is no backup file or the backup file was empty
    //RTC :
